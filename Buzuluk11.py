        ...
        if rows:
            ...
            for item in self.tree.get_children():
                ...
                if values[11] == "Готово к отправке" and REO_Data and stat:
                    self.tree.set(item, column="Статус от объекта 1", value="Отправлено")
                    self.tree.set(item, column="Статус от объекта 2", value="Не отправлено")
                    self.tree.set(item, column="Дата отправки",
