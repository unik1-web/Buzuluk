...
class App(tk.Tk):
    ...
    def create_additional_json(self, temp_data):
        ...
        conn = sqlite3.connect('app_data.sqlite')
        cursor = conn.cursor()
        cursor.execute("SELECT nomer_ts2, marka_ts2, min_weight FROM list_auto")
        transport_data = cursor.fetchall()
        conn.close()

        # Создание словаря для быстрого поиска данных по номеру авто
        transport_dict = {nomer: (marka, min_weight) for nomer, marka, min_weight in transport_data}
        ...
        for weight_control in temp_data["WeightControls"]:
            ...
            print(numbers)
            registration_number2 = next(cycle(numbers))  # Используем циклический итератор
            marka_ts2, min_weight = transport_dict[registration_number2]
    ...
    def update_table(self, rows):
        ...
        for row in rows:
            ...
            formatted_row[11], formatted_row[12], formatted_row[13] = [status, status, row[12]] # Добавляем статус и пустую дату отправки
        ...
        if rows:
            ...
            for row in rows:
                ...
                if result[0] > 0:
                    ...
                    self.tree.set(item, column=12, value="Уже отправлено")  # Используем индекс 12
            ...
            for item in self.tree.get_children():
                ...
                if values[11] == "Готово к отправке" and REO_Data:
                    self.tree.set(item, column=11, value="Отправлено")  # Используем индекс 11
                    self.tree.set(item, column=12,
                                  value=datetime.now().strftime("%Y-%m-%d %H:%M:%S"))  # Используем индекс 12
...
class AutoWindow(tk.Toplevel):
    ...
    def create_table(self):
        ...
        for col in columns:
            self.tree.heading(col, text=col)
            self.tree.column(col, width=150)  # Устанавливаем ширину колонок
        self.tree.pack(fill=tk.BOTH, expand=True)
...
class CarIterator:
    def __init__(self):
        ...
        # Создание словаря для быстрого поиска данных по номеру авто
        transport_dict = {nomer: (marka, min_weight) for nomer, marka, min_weight in transport_data}
        self.cars = [transport_dict]
        #     {"номер авто": "А123ВС", "минимальная масса": 1000, "максимальная масса": 1500},
        #     {"номер авто": "Б456ДЕ", "минимальная масса": 1200, "максимальная масса": 1700},
        #     {"номер авто": "В789ФГ", "минимальная масса": 1100, "максимальная масса": 1600}
        # ]
        print(self.cars)
        self.index = 0
