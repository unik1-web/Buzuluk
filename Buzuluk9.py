...
class App(tk.Tk):
    ...
    #         if sender not in company_dict:  # Пропускаем строку, если отправитель отсутствует в списке
    #             continue
    #
    #         inn, kpp = company_dict[sender]  # Получаем ИНН и КПП из словаря
    #
    #         # Добавляем ИНН и КПП в строку данных
    #         formatted_row.extend([inn, kpp])  # Добавляем ИНН и КПП в конец строки
    #
    ...
    #             tag = ""
    #
    #         formatted_row.extend([status, ""])  # Добавляем статус и пустую дату отправки
    #
    #         # Вставляем строку в таблицу
    #         item = self.tree.insert("", tk.END, values=formatted_row, tags=(tag,))
    #
    ...
    def create_json_structure(self, rows):
        ...
        object_id = self.settings.get("object_id", "insert_ObjectId")  # Идентификатор объекта
        access_key = self.settings.get("access_key", "insert_AccessKey")  # Ключ доступа

        data = {
            "ObjectId": object_id,  # Используем идентификатор объекта из настроек
        ...
        for row in rows:
            ...
            date_before = datetime.strptime(row[0], "%Y-%m-%d %H:%M:%S").isoformat() + "+03:00"  # if row[0] else None
            date_after = datetime.strptime(row[1], "%Y-%m-%d %H:%M:%S").isoformat() + "+03:00"  # if row[1] else None

            weight_control = {
                "Id": uid,
                "DateBefore": date_before,  # .strftime("%Y-%m-%d %H:%M:%S+05:00") if date_before else None,
                "DateAfter": date_after,  # .strftime("%Y-%m-%d %H:%M:%S+05:00") if date_after else None,
    ...
    def send_data(self):
        ...
        if rows:
            ...
            for row in rows:
                ...
                date_before = datetime.strptime(row[0], "%Y-%m-%d %H:%M:%S").isoformat() + "+05:00"
                date_after = datetime.strptime(row[1], "%Y-%m-%d %H:%M:%S").isoformat() + "+05:00"
            ...
            send_to_reo(json_data, self.url)
            ...
            for item in self.tree.get_children():
                values = self.tree.item(item, "values")
                if values[11] == "Готово к отправке":
                    self.tree.set(item, column="Статус отправки в РЭО", value="Отправлено")
...
class SettingsWindow(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.object_id = tk.StringVar(value=self.settings.get("object_id", ""))
        self.object_name = tk.StringVar(value=self.settings.get("object_name", "Объект обработки TKO, г. Бузулук"))
    ...
    def create_widgets(self):
        ...
        db_entry = ttk.Entry(settings_frame, textvariable=self.db_path, width=40)
        ...
        weight_combobox = ttk.Combobox(settings_frame, textvariable=self.weight_format, values=["#.", "#.#", "#.##"])
        weight_combobox.grid(row=1, column=1, padx=10, pady=10)

        # Дата и время
        ttk.Label(settings_frame, text="Формат даты и времени:").grid(row=2, column=0, padx=10, pady=10, sticky=tk.W)
        date_combobox = ttk.Combobox(settings_frame, textvariable=self.date_format,
                                     values=["%Y-%m-%d %H:%M:%S", "%d.%m.%Y %H:%M:%S", "%Y/%m/%d %H:%M:%S"])
        date_combobox.grid(row=2, column=1, padx=10, pady=10)

        # Ключ доступа
        ttk.Label(settings_frame, text="Ключ доступа:").grid(row=3, column=0, padx=10, pady=10, sticky=tk.W)
        access_key_entry = ttk.Entry(settings_frame, textvariable=self.access_key, show="*", width=60)
        access_key_entry.grid(row=3, column=1, padx=10, pady=10)

        # Идентификатор объекта
        ttk.Label(settings_frame, text="Идентификатор объекта:").grid(row=4, column=0, padx=10, pady=10, sticky=tk.W)
        object_id_entry = ttk.Entry(settings_frame, textvariable=self.object_id, width=60)
        object_id_entry.grid(row=4, column=1, padx=10, pady=10)

        # Наименование объекта
        ttk.Label(settings_frame, text="Наименование объекта:").grid(row=5, column=0, padx=10, pady=10, sticky=tk.W)
        object_name_entry = ttk.Entry(settings_frame, textvariable=self.object_name, width=60)
        object_name_entry.grid(row=5, column=1, padx=10, pady=10)

        # Адрес сервиса
        ttk.Label(settings_frame, text="Адрес сервиса:").grid(row=6, column=0, padx=10, pady=10, sticky=tk.W)
        object_url_entry = ttk.Entry(settings_frame, textvariable=self.object_url, width=60)
        object_url_entry.grid(row=6, column=1, padx=10, pady=10)

        # Фрейм для кнопок проверки
        check_buttons_frame = ttk.Frame(settings_frame)
        check_buttons_frame.grid(row=7, column=1, padx=10, pady=10, sticky=tk.W)
        ...
        self.description_label.grid(row=9, column=0, columnspan=3, padx=10, pady=10)
        ...
        db_entry.bind("<FocusIn>", lambda e: self.show_description("Выберите файл базы данных для подключения."))
        weight_combobox.bind("<FocusIn>", lambda e: self.show_description("Выберите формат вывода веса."))
        date_combobox.bind("<FocusIn>", lambda e: self.show_description("Выберите формат вывода даты и времени."))
        access_key_entry.bind("<FocusIn>", lambda e: self.show_description("Введите ключ доступа для отправки данных."))
        object_id_entry.bind("<FocusIn>", lambda e: self.show_description("Введите идентификатор объекта."))
        object_name_entry.bind("<FocusIn>", lambda e: self.show_description("Введите наименование объекта."))
        object_url_entry.bind("<FocusIn>",
