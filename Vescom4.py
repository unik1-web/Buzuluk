...
def manage_senders():
    ...
    senders_window.geometry("400x300")

    # Поля для ввода ИНН и КПП
    tk.Label(senders_window, text="ИНН:").grid(row=0, column=0, padx=5, pady=5)
    inn_entry = tk.Entry(senders_window)
    inn_entry.grid(row=0, column=1, padx=5, pady=5)

    tk.Label(senders_window, text="КПП:").grid(row=1, column=0, padx=5, pady=5)
    kpp_entry = tk.Entry(senders_window)
    kpp_entry.grid(row=1, column=1, padx=5, pady=5)

    # Кнопка для сохранения
    def save_sender():
        inn = inn_entry.get()
        kpp = kpp_entry.get()
        messagebox.showinfo("Сохранено", f"Отправитель с ИНН {inn} и КПП {kpp} сохранен.")
        senders_window.destroy()

    tk.Button(senders_window, text="Сохранить", command=save_sender).grid(row=2, column=0, columnspan=2, pady=10)

def manage_cargo_types():
    # Окно для управления типами груза
    cargo_window = tk.Toplevel(root)
    cargo_window.title("Управление типами груза")
    cargo_window.geometry("400x300")

    # Поле для ввода типа груза
    tk.Label(cargo_window, text="Тип груза:").grid(row=0, column=0, padx=5, pady=5)
    cargo_entry = tk.Entry(cargo_window)
    cargo_entry.grid(row=0, column=1, padx=5, pady=5)

    # Кнопка для сохранения
    def save_cargo():
        cargo_type = cargo_entry.get()
        messagebox.showinfo("Сохранено", f"Тип груза '{cargo_type}' сохранен.")
        cargo_window.destroy()

    tk.Button(cargo_window, text="Сохранить", command=save_cargo).grid(row=1, column=0, columnspan=2, pady=10)
...
lists_menu.add_command(label="Типы груза", command=manage_cargo_types)
